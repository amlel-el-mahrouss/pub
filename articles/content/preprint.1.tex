...
\end{equation}
And consequently additional remarks and observations regarding tensor products and their representations over Riemannian Manifolds are discussed using the Fubini's Theorem and a proof by construction.
...
\subsection{Tensor and Set Definitions:}

Let a tensor $\mathcal{X}$ be defined in:
\begin{equation}
  \mathcal{X} \subset \mathcal{T}_i \cdot \mathcal{T}_{ij} \cdot \mathcal{T}_{ijk}.
\end{equation}
For the following tensor $\mathcal{X}$ to be defined in:
\begin{equation}
  \mathcal{X} \in \mathbb{T}^{+}.
\end{equation}
Where $\mathbb{T}^{+}$ is the set of all non-zero three-dimensional tensors.

\subsection{Mathematical Definitions of a Riemannian Manifold Product Tensor:}
...
(i) By Corollary~\ref{cor:tensor-nz}, the contraction $\mathcal{T}_i \cdot \mathcal{T}_{ij} \cdot \mathcal{T}_{ijk}$ is non-degenerate on $\mathcal{P}_{S,F}$, since the constituent tensors are assumed non-zero by Definition~\ref{def:two-one}. In particular, $\langle \mathcal{P} \rangle \neq 0$ on the initial leaf $\mathcal{P}_S$.\\
(ii) By Corollary~\ref{cor:three}, $\mathcal{F}_t(i,j,k)$ factors as $\mathcal{T}_i(t) \cdot \mathcal{T}_{ij}(t) \cdot \mathcal{T}_{ijk}(t)$ on each leaf $\mathcal{P}_t$. Suppose the identity $\langle \mathcal{P} \rangle = \int_{\mathcal{P}_{S,t}} \mathcal{F}_t(i,j,k)\,d\mathrm{vol}$ holds for all leaves up to $\mathcal{P}_t$. Since $\mathcal{F}_t$ is continuous and non-degenerate by Corollaries~\ref{cor:tensor-nz} and~\ref{cor:three}, the identity extends to $\mathcal{P}_{t+\epsilon}$ for any $\epsilon > 0$ by continuity of the foliation parameter.\\
