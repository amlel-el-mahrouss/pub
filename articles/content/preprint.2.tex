...
\end{equation}
\begin{example}
  Let the Equation~\ref{eq:integral:1} in $\mathbb{R}$, within an interval of $\mathbb{R}^{+}$ to $+\infty$:
  \begin{equation}
      - \int_{\mathbb{R}^{+}}^{+\infty} \nabla p + \nabla \cdot \tau + \rho \, \textbf{a} \, d\tau.
  \end{equation}
  Applying the decomposition theorem for the Equation~\ref{eq:integral:1} for the two intervals $[\mathbb{R}^{+}, x]$ and $[x, +\infty]$:
  \begin{equation}
      - \int_{\mathbb{R}^{+}}^{x} \nabla p + \nabla \cdot \tau + \rho \, \textbf{a} \, d\tau \pm \int_{x}^{+\infty} \nabla p + \nabla \cdot \tau + \rho \, \textbf{a} \, d\tau.
  \end{equation}
  ...
  \end{equation}
  The limit has been invoked per \ref{eq:integral:1} to establish non-degeneracy cases for \ref{eq:cauchy} and \ref{eq:integral:1}. Moreover tigheting the interval of values would turn out to server us well in the next sections about applied examples of the integral.
\end{example}
